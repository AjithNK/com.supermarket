...
public class MobileSliderTest extends BaseClass {
	...
	public void createNewListItem() throws IOException {
	...
	public void cancelNewItemCreation() throws IOException {
	...
	public void changeStatusToActive() throws IOException, InterruptedException {

		msObj = new MobileSliderPage(driver);
		msObj.changeToActiveStatus();
		String actualStatus = msObj.checkActiveStatus();
		Assert.assertEquals(actualStatus, Constants.expectedActiveStatus,
				"Change mobile slider status from Inactive to Active failed!");
	}

	@Test(priority = 4, enabled = true, description = "This test case will change status of mobile slider from Active to Inactive")
	public void changeStatusToInActive() throws IOException, InterruptedException {

		msObj = new MobileSliderPage(driver);
		msObj.changeToInActiveStatus();
		String actualStatus = msObj.checkInactiveStatus();
		Assert.assertEquals(actualStatus, Constants.expectedInactiveStatus,
				"Change mobile slider status from Active to Inactive failed!");
	}

	@Test(priority = 5, enabled = true, description = "This test case will verify editing of created mobile slider")
	public void editListItem() throws IOException {
